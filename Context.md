...
- **Mermaid: shipped, CDN-free.** `mermaid` fenced code blocks render as embedded SVG diagrams via a `marked` renderer override. The chosen renderer library (`beautiful-mermaid`) turned out to bake an unconditional Google Fonts CDN `@import` into its own generated output — found and stripped during implementation, not assumed absent; see `docs/adr/0005-mermaid-local-cdn-import-stripped.md`. **KaTeX: explicitly deferred, not silently skipped.** Decided-but-not-yet-installed on this branch — a documented fast-follow tracked separately (see Roadmap below), not a gap anyone should quietly work around (e.g. via a CDN script tag, which would violate the local-first constraint on its own).
...
- **The local-first constraint has now been tested against a real "convenience" pressure, and held.** The Mermaid implementation's chosen renderer library (`beautiful-mermaid`) turned out to bake a hardcoded Google Fonts CDN `@import` into its own default output — a real proposed CDN dependency, not a hypothetical one, discovered by re-checking rendered output content rather than just the npm dependency tree. It was found and stripped rather than shipped; see `docs/adr/0005-mermaid-local-cdn-import-stripped.md`. This resolves the risk as originally stated for Mermaid; KaTeX (still deferred on this branch) has not yet had the equivalent check run against this branch's own history.
