...
│   ├── render.ts                  generateHtml(markdown, title?, customCss?, themeColors?, transitionName?, cssVars?) -> HTML string, via `marked`
...
│   ├── presentationScript.ts      PRESENTATION_SCRIPT — client-side ?present nav: keyboard/click/touch, grid overview, progress bar, "?" help overlay, fragment reveal
│   ├── fragments.ts               extractFragments(tokens) — <!-- fragment --> marker extraction/binding for incremental per-element reveal
...
│   ├── mermaidRenderer.ts         renderMermaidDiagram(code, colors?, diagramIndex?) — CDN-free, theme-recolorable Mermaid SVG via `beautiful-mermaid`, optimized/id-prefixed via `svgo`
...
| `src/render.ts` | The render module. Exports `generateHtml(markdown, title?, customCss?, themeColors?, transitionName?, cssVars?): string`, converting a deck's Markdown source into a self-contained HTML string via `marked`. | Pure function, no filesystem or network I/O — this is what makes it the wide base of the test pyramid (see `TESTING.md`). KaTeX math, theme-recolorable Mermaid diagrams, per-slide layouts, presentation mode, transitions, the grid overview, the `?` help overlay, fragment/incremental reveal, and the `--css-vars` layered overlay have all landed here. |
...
| `src/presentationScript.ts` | Exports `PRESENTATION_SCRIPT` — the client-side JS (as a `<script>`-wrapped string) for the opt-in `?present` one-slide-at-a-time navigation mode. | Entirely inert unless `?present` is in the URL; keyboard/click/touch-swipe nav, `location.hash` persistence across `--watch` reloads, a grid overview mode (`o`/Escape), a `?`-triggered keyboard-shortcut help overlay, and the fragment-reveal forward/backward state machine — all composing under one documented keypress-precedence order. |
| `src/fragments.ts` | Exports `extractFragments(tokens: Token[]): { tokens, hasFragment }` — walks a slide's token tree generically (mirroring `render.ts`'s `containsUnsafeHtml` walk) to bind a standalone `<!-- fragment -->` comment to its adjacent paragraph/list-item/blockquote/code-fence/diagram. | `hasFragment` lets `generateHtml` skip the reduced-motion CSS cost for slides with no fragments, the same way it already checks for `class="katex"` before paying KaTeX's font cost. |
...
| `src/mermaidRenderer.ts` | Exports `renderMermaidDiagram(code: string, colors?: ThemeColors, diagramIndex?: number): string` — renders Mermaid source to a CDN-free, theme-recolorable, svgo-optimized SVG string via `beautiful-mermaid`, or an escaped error box on invalid syntax. | Strips a real Google Fonts CDN `@import` that `beautiful-mermaid` bakes into its own default output; see `docs/adr/0005-mermaid-local-cdn-import-stripped.md`. `diagramIndex` (supplied by `render.ts`, incrementing per mermaid block) becomes svgo's `prefixIds` prefix, so 2+ diagrams in one document never collide on ids like `arrowhead`. |
| `src/server.ts` | The local dev server. A plain `node:http` server (no framework) that serves the rendered HTML on an ephemeral port, prints the serving URL to stdout, honors `--no-open`, and gzip-compresses responses when the client's `Accept-Encoding` allows it (a real RFC 7231 evaluator, not a prefix match). | No routing, no middleware — it serves exactly one already-rendered HTML string per invocation. |
| `src/pdfExport.ts` | The PDF export module. Uses `chrome-launcher` to detect a local Chrome-family browser, then drives it via `puppeteer-core` to print the rendered HTML to a landscape 16:9 PDF file with page numbers. | Must fail with a clear, non-crashing error (not a stack-trace crash) if no local browser is found — no auto-download fallback in this skeleton. |
...
| `src/pngExport.ts` | Exports `exportToPng(html, outputPath): Promise<string[]>` — locks a fixed 1280x720 viewport, then renders the given HTML in a locally-detected browser and screenshots each `<section class="slide">` to its own PNG file. | Depends on `src/browserLaunch.ts` for browser detection; file names are derived by inserting `-N` (1-indexed, zero-padded to the total slide count's digit width, e.g. `-01`..`-12`) before `outputPath`'s extension. |
...
| `package.json` | Declares dependencies (Commander.js, `marked`, `puppeteer-core`, `chrome-launcher`, `beautiful-mermaid`, `katex`, `marked-katex-extension`, `svgo`, `open`) and npm scripts (`build`, `test`, `test:watch`, `lint`, `typecheck`, `check:fidelity`). | Both KaTeX and Mermaid's runtime dependencies are listed here now — see `tech.md`. |
...
| Change presentation-mode navigation (keyboard/click/touch, hash persistence, overview mode, help overlay) | `src/presentationScript.ts` |
| Change fragment/incremental-reveal marker extraction or binding | `src/fragments.ts` |
