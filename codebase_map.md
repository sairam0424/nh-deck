...
| `package.json` | Declares dependencies (Commander.js, `marked`, `puppeteer-core`, `chrome-launcher`, `beautiful-mermaid`) and npm scripts (`build`, `test`, `test:watch`). | KaTeX is explicitly **not** listed here yet — see `tech.md`. Mermaid (`beautiful-mermaid`) now is. |
...
| Change how Markdown becomes HTML (Mermaid diagrams already added; KaTeX still pending) | `src/render.ts` |
