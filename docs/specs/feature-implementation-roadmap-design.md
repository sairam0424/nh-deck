...
**Re-verification completed** (in a throwaway `/tmp` sandbox, not the repo) before writing the implementation plan, per this section's own prior instruction not to treat that check as already done:

- `beautiful-mermaid@1.1.3`'s dependency tree is confirmed still minimal (`elkjs@^0.11.0`, `entities@^7.0.1` only — no browser, no DOM, no Puppeteer). Core `mermaid@12.0.0` was confirmed genuinely heavy/browser-oriented (D3, DOMPurify, cytoscape, roughjs) and `@mermaid-js/mermaid-cli` still requires full `puppeteer`. `beautiful-mermaid` remains the correct choice.
- **A real, previously-undocumented anti-pattern was found by this re-verification, not by the original research**: `beautiful-mermaid`'s rendered SVG output contains a hardcoded, unconditional `@import url('https://fonts.googleapis.com/css2?family=Inter...')` (and a second conditional import for JetBrains Mono when a monospace font is configured) — a genuine external CDN fetch inside the library's own generated CSS, sourced from its `buildStyleBlock()` function with no built-in disable option. This is exactly the class of anti-pattern §6's opening paragraph warns about, confirming the research's instinct to require one more concrete look before committing.
- **Fix, empirically verified**: the generated CSS rule is already `font-family: 'Inter', system-ui, sans-serif;` — a proper fallback chain. Implementation must strip every `@import url(...);` occurrence from the raw SVG string returned by `renderMermaidSVG` (a single regex pass, e.g. `/@import url\([^)]*\);?\s*/g`) before embedding it in `generateHtml`'s output. Verified directly: after stripping, the only remaining `http://`-shaped string in the SVG is the standard, non-fetched `xmlns="http://www.w3.org/2000/svg"` namespace declaration (the same category of false positive as KaTeX's MathML namespace, already handled by the existing hostname-specific CDN test pattern) — confirmed via a real headless-browser load showing zero network requests beyond the local file, and a screenshot confirming the diagram still renders correctly with the system-font fallback.
- `renderMermaidSVG` has no `throwOnError`-style graceful-degradation option (unlike KaTeX) — it throws a plain `Error` for invalid Mermaid syntax (e.g. `"Invalid mermaid header: ..."`). Implementation must wrap the render call in its own try/catch and render a visible, escaped error message in place of the diagram, rather than letting `generateHtml` throw and take down the whole render — this is nh-deck's own equivalent of KaTeX's `throwOnError: false`, not a library-provided one.
- Integration point confirmed against the installed `marked@13.0.3` (same version already used by 4a's KaTeX extension): `marked.use({ renderer: { code({ text, lang, escaped }) { ... } } })`. Mermaid diagrams use standard fenced-code syntax with a language tag (` ```mermaid `), not new delimiter syntax, so no tokenizer extension is needed — only a renderer override on the existing `code` token. The override must replicate marked's own default fenced-code-block output (`<pre><code class="language-...">...</code></pre>`, escaping via the same rules) for every `lang` other than `"mermaid"`, to avoid regressing rendering for all other fenced code blocks in every existing and future deck.
...
**Testing:** golden-file test for Mermaid SVG output from a fixture `.md` with a `mermaid` code fence; same zero-CDN/zero-Puppeteer-in-process assertion pattern as 4a, extended to explicitly assert the `@import url(` stripping actually ran (not just that no CDN hostname appears, since the two are different failure modes); a test asserting non-`mermaid` fenced code blocks render identically to before this phase; and a test for the invalid-syntax error-box path.
