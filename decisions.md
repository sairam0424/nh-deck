...
| 2026-09-02 | Build with plain `tsc` (transpile-only), no bundler.                                                            | Matches the reference project's (`arpitbbhayani/deckrun`) "no bundler" philosophy; a small CLI does not need bundling to ship `dist/index.js`. | @sairamugge |
